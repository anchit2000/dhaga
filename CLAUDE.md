...
- Every AI feature is metered per user from day one, and the unit is the **user-visible action, not the model call** — wrap an entry point in `withAiAction(feature, fn)` (`apps/web/src/lib/ai/metering`) and every `recordAiAction` inside it folds into one `ai_actions` row whose tokens are the sum. A new AI call site added inside an existing action is therefore correct by default; a new *entry point* needs its own `withAiAction` and a credit price in `packages/core/src/metering/credits.ts`. Cost is priced in **AI credits** (1 credit ≈ one card scan; see BRD §8.3 for the measured table). Every plan carries a monthly allowance and enforcement ships ON (`AI_PLAN_CAP_ENFORCEMENT_DEFAULT`); free gets 10 credits/month. `DHAGA_AI_MONTHLY_CAP` (credits/month) *seeds* the instance default until an admin sets one — see `lib/ai/metering/cap/index.ts` for the full precedence. A **second, independent ceiling in inference dollars** (`lib/ai/metering/dollar-cap.ts`, on by default) gates *every* action including the ones priced at 0 credits, since credits alone no longer bound spend; credits are checked first (that's the message a user can act on), dollars second. It resolves to *no ceiling* where no plan is in play, so it's inert on a core-only self-host
