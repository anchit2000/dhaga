...
anywhere, being operator config rather than user data. The cap resolves exactly
as it did before: whatever `DHAGA_AI_MONTHLY_CAP` says, else the free-tier
constant. **Nothing here is required from `packages/ee` to self-host**, and the
Level 2 removal list above needs no additions — the new admin page and its
