...
import type {
  CardImageErrorResponse,
  CardImageParams,
} from "@dhaga/core/src/api/card-image";
...
export async function GET(
  request: Request,
  { params }: { params: Promise<CardImageParams> },
): Promise<Response> {
  try {
    await requireUserIdFromRequest(request);
  } catch {
    return Response.json(
      { error: "Not signed in to Dhaga." } satisfies CardImageErrorResponse,
      { status: 401 },
    );
  }
  ...
  if (!image) {
    return Response.json({ error: "Not found." } satisfies CardImageErrorResponse, { status: 404 });
  }
