    ...
    >
      {openFollowUps.length === 0 ? (
        <div className="flex flex-1 flex-col justify-center py-6 text-center">
          <p className="text-sm text-paper">You&apos;re caught up.</p>
        ...
        <div className="divide-y divide-seam">
          {openFollowUps.map((item) => (
            <div key={item.id} className="flex items-center gap-2.5 py-2 first:pt-0 last:pb-0">
              ...
              <p className="min-w-0 flex-1 truncate text-sm text-paper">{item.action}</p>
              <Button render={<div />} variant="ghost" onClick={() => onSelectContact(item.contactId)} className="h-auto shrink-0 rounded-md p-0 text-xs font-normal normal-case text-amber hover:bg-transparent hover:underline">{item.contactName}</Button>
            </div>
          ))}
        </div>
      )}
