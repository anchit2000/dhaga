      ...
      <Card>
        ...
        <CardContent className="space-y-1">
          {people.length === 0 ? <p className="py-4 text-sm text-fog">Your newly captured people will appear here.</p> : people.slice(0, HOME_PREVIEW_LIMIT).map((person) => (
            <Button key={person.id} render={<div />} variant="ghost" onClick={() => onSelectContact(person.id)} className="flex h-auto min-h-11 w-full items-center justify-between gap-3 rounded-lg px-2 text-left text-sm font-normal normal-case transition-colors hover:bg-paper/[0.04]">
