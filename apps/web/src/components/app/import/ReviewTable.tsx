...
export function ReviewTable({ candidates, selected, onToggle }: {
  ...
  const columns: DataTableColumn<ReviewRow>[] = [
    ...
    { id: "title", label: "Title", value: (row) => row.candidate.contact.title ?? "", render: (row) => row.candidate.contact.title || "—", className: "text-fog" },
    { id: "company", label: "Company", value: (row) => row.candidate.contact.company ?? "", render: (row) => row.candidate.contact.company || "—", className: "text-fog" },
