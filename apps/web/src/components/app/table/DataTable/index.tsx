...
export function DataTable<Row>({ rows, columns, rowKey, emptyMessage = "No rows match these filters.", initialFilters = {}, server, selection }: {
  rows: Row[];
  ...
  return (
    <div className="space-y-3">
      ...
      >
        <Table>
          <TableHeader>
            <TableRow>{selection ? <SelectionHeadCell pageIds={pageIds} selection={selection} /> : null}{columns.map((column) => {
          ...
          <TableBody>
            {visible.length === 0 ? <TableRow><TableCell colSpan={columns.length + (selection ? 1 : 0)} className="h-24 text-center text-fog">{emptyMessage}</TableCell></TableRow> : visible.map((row) => <TableRow key={rowKey(row)}>
              {selection ? <SelectionBodyCell id={rowKey(row)} selection={selection} /> : null}
