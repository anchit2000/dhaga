 ...
 *
 * The session pooler exposes a FIXED pool_size shared across EVERY warm Vercel
 * instance (~48 = 80% of the instance's 60 max_connections; verify in the
 * Supabase dashboard). Each instance opens this core pool AND, in hosted mode, a
 * separate EE tenant pool (packages/ee/src/db/pool.ts). So the real per-instance
 * draw is (core max + tenant max), and several instances can be warm at once.
 * Keep the sum small enough that a handful of instances still fit under it — the
 * defaults below give 2 + 3 = 5/instance, so ~9 warm instances fit. Do NOT raise
 * these blindly: one instance hoarding the whole pool is exactly the
 * EMAXCONNSESSION outage this guards against.
 *
 * The maxes and both timeouts are env-overridable (DB_POOL_MAX_CORE /
 * DB_POOL_MAX_TENANT / DB_POOL_CONNECTION_TIMEOUT_MS / DB_POOL_IDLE_TIMEOUT_MS)
 * so they can be tuned from Vercel without a redeploy.
 */
 ...
 *  node-postgres counts the FULL acquisition here — including establishing a new
 *  physical connection (TCP+TLS+SCRAM). This pool serves better-auth's per-request
 *  session read (the FIRST DB touch on every request). Intra-region a cold
 *  handshake is ~ms; kept at 10s as headroom for a region-away DATABASE_URL.
 *  Bounds how long we WAIT, not how many slots we hold — no effect on pool_size. */
export const DB_POOL_CONNECTION_TIMEOUT_MS_DEFAULT = 10_000;

/** Default idle timeout; override with DB_POOL_IDLE_TIMEOUT_MS. Keeps an idle
 *  backend this long so a request burst reuses one warm connection. Lowered to
 *  10s now that compute + DB are co-located (cheap reconnect ⇒ no idle-slot
 *  hoarding); min:0 still drains fully between visits. */
export const DB_POOL_IDLE_TIMEOUT_MS_DEFAULT = 10_000;

/**
 * Transient-rejection retry (see lib/db/connect-retry.ts). When several warm
 * instances briefly overshoot the shared pool_size, Supavisor rejects a
