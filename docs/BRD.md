...
| Candidate | Notes |
|---|---|
| **Personal MCP server** — let external AI clients (Claude Desktop/Code, Cursor) query the user's graph and explicitly attach user-supplied notes, cards, or photos to a contact | Backlog. Self-host-friendly and user-scoped. Reads remain private to the connected client; every write is an explicit tool call with a source receipt—never a silent graph mutation |
