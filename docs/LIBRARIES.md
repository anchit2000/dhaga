...
### 17. qrcode.react (scan-to-link for messaging capture) — adopted 2026-08-06

**Powers:** the QR codes in **Settings → Messaging**, which encode a per-channel
deep link carrying the account-linking token — Telegram
`t.me/<bot>?start=<token>`, WhatsApp `wa.me/<digits>?text=<token>`.

**Why not hand-roll it.** QR is Reed–Solomon error correction plus a mask-pattern
search plus a version/capacity table. It is a couple of hundred lines of
error-prone maths whose failure mode is a code that *looks* right and won't scan,
which is exactly the bug you can't see in review. `qrcode.react` renders SVG
declaratively from a string, has no runtime dependency, and is the standard
React binding for it.

**Why client-side, not a server-rendered image.** The token is regenerated by a
button in the same panel. A server-rendered QR would keep showing the previous
code until a round-trip landed — handing someone a code that links nothing. The
component is a pure function of the token in state, so the code and the printed
characters can never disagree.

**Gotcha worth keeping:** the QR uses its own `--brand-qr-plate` /
`--brand-qr-ink` tokens, identical in light and dark. `ink`/`paper` swap between
themes, so deriving from them would invert the code in one of them, and a
light-on-dark QR is unreadable to a good share of scanners. A QR is a machine
target, not a themed surface.
