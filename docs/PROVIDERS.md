...
**Not yet proven.** The `anthropic` provider has never been exercised against a
live key: it typechecks and is unit-tested against recorded response shapes, but
no end-to-end search has been observed. It is also billed twice over — $10 per
1,000 searches *and* every retrieved page charged as input tokens to the
searching model — so the cost figures in BRD §8.3 for the watchlist sweep are
