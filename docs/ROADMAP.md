...
| M3 | Voice + text notes | **Shipped** (Dhaga Voice — on-device Moonshine transcription, WebGPU — + transcript) | Planned — on-device transcription |
...
| Browser extension (Chrome/Edge, one-click capture + "save this article to X") | **Built, not published** — it works today only if you load it unpacked from the repo. The Chrome Web Store listing is packaged but unsubmitted; publishing needs a manual reviewer demo account and the one-time developer registration fee, so there is no install link on the site |
