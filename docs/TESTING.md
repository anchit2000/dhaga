...
- [ ] **Scanning feedback**: from the moment **Scan** is pressed until the
      review dialog opens, the branded loader covers the whole viewport —
      *above* the capture dialog, and identically for a dock **Upload** (which
      scans with that dialog shut). Check it with a full six-image tray, where
      the dialog itself scrolls: the scrim must not scroll away with the form.
