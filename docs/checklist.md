...
- [ ] Admin AI-credit controls (`/app/admin/ai-credits`) — built 2026-07-30, **enforcement built but deliberately not enabled**. Four levers, in the order an operator reasons about them: a **master switch** for plan-cap enforcement (`AI_PLAN_CAP_ENFORCEMENT_DEFAULT = false` — ships OFF, and while it is off nothing about today's behaviour changes: paid plans still resolve through `hasUnlimitedAi` and the pricing page still sells Pro/Annual as "no monthly cap", so turning it on is a *pricing* decision, not a metering one); a runtime-editable **allowance per plan** (`free`/`pro`/`lifetime`/`power` — `power` is sized but not sold — defaulting to `PLAN_AI_CREDITS_PER_MONTH`); an instance-wide **promotional month** that applies regardless of the switch and self-expires (its window is compared at read time, so no cron and no admin cleanup); and an additive **grant** ledger for make-goods, with the same card pinned to one user on `/app/admin/users/[id]` (which also shows "+N granted"). Precedence, pinned by `budget-precedence.test.ts` + `promotion-and-grants.test.ts`: per-user override → promotion → plan allowance (only when enforced, and only when a plan is in play) → `DHAGA_AI_MONTHLY_CAP` → free tier, with active grants added on top (`lib/ai/metering/cap.ts`). A grant **never** edits `ai_actions` — that table is the only record of what cloud AI actually cost, so "End now" ends a grant without deleting its row and recorded usage always stays intact. Storage is two new instance-wide core tables (`ai_budget_settings`, `ai_credit_grants` — `lib/db/ddl/ai-budget.ts`), deliberately **not** in the tenant-scoped `settings` table or in EE's `TENANT_TABLES`: `ai_credit_grants` gets a bespoke RLS policy in `packages/ee` (`user_id IS NULL OR user_id = <tenant>`) because the generic policy would hide instance-wide grants from everyone, and `ai_budget_settings` gets none (operator config). Grant writes live in `packages/ee/src/admin/ai-grants.ts`; a core-only self-host keeps both tables, gets no admin UI to write them, and resolves the cap from `DHAGA_AI_MONTHLY_CAP` / the free-tier constant exactly as before. tsc/lint/vitest clean; **manual click-through pending**, same bar as the rest of this section
