...
- [x] Free-tier cap enforced (0/month — cloud AI is a paid feature; `DHAGA_AI_MONTHLY_CAP` override for self-hosters) with clear UI message. The cap is now denominated in **AI credits**, priced per action from real measured cost (BRD §8.3, `packages/core/src/metering/credits.ts`): 1 credit = a card scan, note, quick add or draft; 2 = Ask Dhaga or a brief; 20 = deep research; 0 = a nightly watchlist scan (throttled by the watch limit instead)
