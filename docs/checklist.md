...
- [x] **"Dhaga Voice" (Moonshine tiny) engine, web** — the only web STT engine; on-device WebGPU streaming. **WebGPU is required and there is no fallback** — when WebGPU is unavailable, voice is simply unavailable and a **warning toast** is shown (no browser / CPU / WASM degrade). `@huggingface/transformers` dynamic-import-only; COOP/COEP scoped to `/app/**`. *(typecheck + prod build pass; **pending real-browser mic/WebGPU verification**)*
