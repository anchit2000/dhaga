...
export const AI_ACTION_CREDITS: Record<AiActionFeature, number> = {
  ...
  goal_match_now: 3,
  /**
   * Planning one forwarded messaging batch: a single Haiku call that reads
   * every message in the batch together and returns who it is about and what
   * to store on each of them.
   *
   * This REPLACED a per-message `contact_parse` call. A five-message batch used
   * to spend five credits parsing five messages in isolation (and still got the
   * attribution wrong, because no call ever saw more than one of them); it now
   * spends one. So this line makes multi-message batches cheaper, not dearer,
   * and the single-message batch is a wash at 1 credit either way.
   *
   * ESTIMATED, not measured. The table at the top of this file is 39 real API
   * calls; this number has never been run against the live API, so re-derive it
   * the first time it is. The estimate, for a mid-sized batch:
   *
   *   ~5 messages × ~200 tok + ~600 tok system + ~200 tok candidates ≈ 1,800 in
   *   a two-person plan with note bodies echoed back              ≈   400 out
   *   Haiku 4.5 at $1/$5 per MTok:
   *     in  1,800 × $1 / 1e6 = $0.0018
   *     out   400 × $5 / 1e6 = $0.0020
   *     total                = $0.0038
   *   $0.0038 ÷ ~$0.006 per credit (the blended ceiling above) ≈ 0.63 → 1.
   *
   * Input scales with batch size, and MESSAGING_MAX_OPEN_ITEMS caps a batch at
   * 10 messages — so the worst case is roughly double the estimate above and
   * still inside one credit's ~$0.006. Re-check this first if that cap is
   * raised, since this is the one action whose cost the SENDER controls.
   */
  batch_plan: 1,
};
